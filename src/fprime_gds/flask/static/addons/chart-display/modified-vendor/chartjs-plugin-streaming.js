...
function setRealtimeScale(scale, {min, max}, limits, zoom = false) {
  const {chart, axis} = scale;
  const currentMax = scale.max;
  const {minDelay = -Infinity, maxDelay = Infinity} = limits && limits[axis] || {};
  const {minDuration = -Infinity, maxDuration = Infinity} = limits && limits[axis] || {};
  const currentDelay = resolveOption(scale, 'delay');
  storeOriginalScaleOptions(chart);
  scale.options.realtime.delay = clamp((currentMax - max) + currentDelay, minDelay, maxDelay);
  scale.options.realtime.duration = clamp(max-min, minDuration, maxDuration);
  return true;
}
